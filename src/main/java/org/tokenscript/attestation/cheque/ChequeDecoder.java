...
public class ChequeDecoder implements AttestableObjectDecoder<Cheque> {
  ...
  public Cheque decode(byte[] encoding) throws IOException {
    ...
    try {
      ...
      ASN1GeneralizedTime notValidBeforeEnc = ASN1GeneralizedTime.getInstance(
          validity.getObjectAt(0));
      ASN1GeneralizedTime notValidAfterEnc = ASN1GeneralizedTime.getInstance(
          validity.getObjectAt(1));
      ...
      AsymmetricKeyParameter publicKey = SignatureUtility.restoreDefaultKey(
          DERBitString.getInstance(asn1.getObjectAt(1)).getEncoded());
