...
contract VerifyTicket {
    ...
    function verifyTicketAttestation(bytes memory attestation, address attestor, address ticketIssuer) external view returns(address subject, bytes memory ticketId, bool timeStampValid)
    {
        ( subject, ticketId, timeStampValid ) = _verifyTicketAttestation( attestation, attestor, ticketIssuer);
    }

    function _verifyTicketAttestation(bytes memory attestation, address attestor, address ticketIssuer) public view returns(address subject, bytes memory ticketId, bool timeStampValid)
    {
    ...
    function decodeTimeBlock(bytes memory attestation, uint256 decodeIndex) public view returns (uint256 index, bool valid)
    {
        ...
        (length, timeBlock, decodeIndex, tag) = decodeElement(attestation, decodeIndex + length); //02 06
        if (tag == INTEGER_TAG && length == ATTESTATION_TIME_BYTES)
        {
